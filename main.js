...
function calculateTopGainer30d(ranking, historyData) {
    ...
    if (displayEl) displayEl.innerHTML = topUser.delta > 0 ? `<span style="color:#4dff91;">@${topUser.name}</span> <small>(+${Math.floor(topUser.delta)} HP)</small>` : "—";
}
...
function renderTable(data) {
    ...
    data.forEach(user => {
        ...
        row.innerHTML = `
            ...
            <td>${user.in_curation_trail ? '<span class="bonus-tag">+5%</span>' : '—'}</td>
...
function renderRecentActivity(delegations, historyData) {
    ...
    if (changes.length > 0) {
        ...
        tbody.innerHTML = changes.slice(0, 10).map(c => `<tr><td>@${c.name}</td><td>${Math.floor(c.old)}</td><td>${Math.floor(c.new)}</td><td style="color:${c.diff>0?'#4dff91':'#ff4d4d'}">${c.diff>0?'+':''}${Math.floor(c.diff)} HP</td></tr>`).join('');
    }
}
